...
class REST_Controller extends CI_Controller
{
	...
	*/
	public function __construct($config = 'rest')
	{
		...
		if(isset($_POST['lang']))
		{
			if(array_key_exists($_POST['lang'],$this->lang_array()))
			{
		...
		$this->lang->load('rest_controller', $this->config->item('language'), FALSE, TRUE, __DIR__ . '/../');
	...
	*/
	protected function _detect_api_key()
	{
		...
		if(($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
		{
			...
			$this->rest->key = $row->
			{
				$this->config->item('rest_key_column')
			};
	...
	*/
	protected function _perform_library_auth($username = '', $password = NULL)
	{
		...
		{
			$auth_library_class
		}->$auth_library_function($username, $password);
	}
	...
	* @return bool
	*/
	protected function userValid($array)
	{
		if(isset($array['phone']) && isset($array['code']))
		{

			if($this->Crudmodel->get_row(['phone' => $array['phone'],
						'code' => $array['code']], 'customers'))
			return true;
		}
		return false;
	}
