...
class Crudmodel extends CI_Model
{
	...
	*/
	...
	public function city_with_count()
	{
		
		$cities = $this->Crudmodel->get_joins(
			'cities',[
				"cities_translate"=>"cities.id=cities_translate.city_id and cities_translate.lang_code = '".$this->config->item('language')."'",
				"points"=>"points.city_id=cities.id",

			],

			"cities.* ,GROUP_CONCAT(DISTINCT points.id ) as points , cities_translate.title as trans_title ",NULL,['cities.id'],[]);

		$city = [];
		foreach($cities as  $row){
			$city[$row['id']] =
			[
				'title'=>($row['trans_title'] != null) ? $row['trans_title'] : $row['title'],
				'points'=>  $row['points'] == null ? null :  explode(',',$row['points'])
			];
		}
		return $city;
